...
> **v1.0 → v1.1 变更**：Server 架构门审整改——WorldSlotHost 聚合根与生命周期 epoch（ADR-001）、Runtime 见证的 FaultClass 故障分级（ADR-006）、Audit/Journal 分权与 correlation 作用域分层（ADR-011）、维护 deadline 时钟域与外部控制面/fencing（ADR-012）、§16 Server 子模块地图更新；NativeCore 契约分层与 Root 符号归属澄清（ADR-006）：Root 归 CoreEngine root-abi，§16 NativeCore 模块地图更新。字段级变更见各 Schema 与 Fixture。
