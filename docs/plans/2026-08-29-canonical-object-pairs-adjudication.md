...
**更正后的通则**:**拼接式编码单射的充分条件只有一条 —— 出现在分隔符之前的每个字段,自身不得含分隔符。** 分隔符**之后**的字段含分隔符完全无害。

证明:设 `s₁+D+k₁ = s₂+D+k₂`。因 `s₁` 不含 `D`,该串中 `D` 的首次出现必在位置 `|s₁|`;同理在 `|s₂|`。同一个串 ⇒ 首次出现位置相同 ⇒ `|s₁|=|s₂|` ⇒ `s₁=s₂` ⇒ `k₁=k₂`。∎(穷举实证:前提成立时碰撞数 0;允许 `s` 含分隔符时碰撞数 6,如 `("", ":")` 与 `(":", "")` 都拼成 `"::"`。)
...
| 带分隔符 `a+D+b` | **分隔符之前**的字段不含 `D` | 字段字母表 | Runtime durable key(`schemaId` 不含 `:`) |
...
**处置**:这三处**不属本裁决范围**(它们是 LumioServer 自己的实现,不是架构仓发布物),但**同源同型**,已由总调度列入待授权立卡清单,建议与本裁决的 Voxel 侧修复同批排。**修法方向**:`:1033` 对 `relative` 做长度前缀,或拒绝含控制字符 / `=` 的路径;`generator_identity_hash` 两文件间插长度前缀,或各自先单独哈希再哈希哈希列表。
