...
> **架构锚点**：`origin/main` `d59afa9`；本地审计分支 `HEAD` `a3f4691`（较 `origin/main` 超前 3 个提交）。快照时本地工作区干净；本文件的本轮修改不计入快照证据。
...
- W0 候选生成可以重复得到稳定 `outputHash`，但 `packages/` 仍是旧 compiler identity；`validate` 退出码为 2，故架构发布可消费性仍为 **0% green**。
...
|---|---|---|
| `LumioGameEngineArchitecture` | `d59afa9` | 本地审计分支为 `a3f4691`，领先 3 个提交；复核时干净 |
...
| `python tools/lumio_contract.py validate` | FAIL，exit 2 | 已发布 Root ABI compiler digest 与锁定 compiler 不一致 |
...
| 修订 | `R-00235`、`R-00240`、`R-00241` | 分开单进程单槽 V1、登出后完整重登录和 graceful quiesce/drain；live migration 与保留窗口只在独立触发和验收后启用 |
| 否决（不执行） | 新建 Rust/C# 第二套公共协议、OperationId namespace、手改 `packages/` 或以本机文件关闭卡 | 这些路径违反 D-009、生成物和证据纪律；本轮不创建对应卡、不关闭现有卡 |

上面四组 Requirement“修订”对象去重后为 22 张：其中 18 张属于 `MS-00001` 的逐卡表，另外四张支撑性卡（`R-00141`、`R-00172`、`R-00174`、`R-00176`）不属于该里程碑的 70 个 `requirementIds`，但需要同步补充边界说明。仅评论的 Voxel 支撑卡 `R-00155` 也不在这 70 个 ID 中；它不进入更新集，只接收客户端三态缺口评论。`R-00279` 同时出现在发送调度组和 C# 组，按对象只计一次。
...
| `R-00237` | backlog | Rust session/admission saga | 条件/后置 | Rust DS 准入主线后置；保留五步/八步顺序，不与 C# harness 的 A1-alpha 证据合并。 |
...
七条审计评论的职责分别是：`R-00260`（profile 命名）、`R-00235`（WorldSlot 单槽边界）、`R-00240`（V1 重登录）、`R-00241`（quiesce 与 live migration 分离）、`R-00279`（Runtime/Server 调度边界）、`R-00141`（D-005 确认记录）和 `R-00155`（客户端 Chunk 三态）。

## 10. 待授权 Workflow 写入账本（提案，不执行）

以下数字按推荐的 **bootstrap profile** 编制；若 Owner 选择 DS V1，卡面和数量必须重新计算。评论是独立写入动作，不能由“更新卡面”隐含代替。

第 8 节同时列出两份仓内计划文档的后续修订建议；它们不是 Workflow 写入，也不计入下表的 33 个动作。本轮只修改了本审计附录。

**关系端点校正**：当前 Workflow 合同的 `POST /schedule/relations` 只接受
`sourceWorkItemId`、`targetWorkItemId` 和 `type=finish_to_start`；它不能把 `R-*`
Requirement 作为关系端点。`/object-links` 当前也只有只读 `GET`。因此下面四张新
Requirement 与既有卡的八个依赖锚点必须写进新卡正文或审计评论中的 displayKey，不能
计为八次“关系写入”；本账本不创建 WorkItem 来绕过这个边界。

该判断按 2026-08-30 读取的公开 OpenAPI 合同核对：`createScheduleRelation` 的
`CreateScheduleRelationRequest` 字段即上述三项，`listObjectLinks` 没有写入方法；
Workflow 编排规范的“需求级依赖”规则也明确只能用正文约定。

| 写入类型 | 数量 | 精确对象或范围 |
|---|---:|---|
| 更新现有 Requirement | 22 | `R-00141`、`R-00172`、`R-00174`、`R-00176`、`R-00214`、`R-00215`、`R-00221`、`R-00222`、`R-00228`、`R-00229`、`R-00231`、`R-00235`、`R-00236`、`R-00240`、`R-00241`、`R-00245`、`R-00260`、`R-00277`、`R-00278`、`R-00279`、`R-00280`、`R-00281`；只追加 profile/边界/依赖说明，不做状态流转。 |
| 关闭/否决现有对象 | 0 | 无；没有一张现有卡具备本轮可复核的关闭或否决证据。 |
| 新建 Requirement | 4 | GameRuntime 复制发送调度、Client Chunk 三态、Architecture D-005 三档 Confirmation Record、Client RTT/2 与离群剔除；新建后由 Workflow 分配 ID。 |
| 新增审计评论 | 7 | 分别挂在 `R-00260`、`R-00235`、`R-00240`、`R-00241`、`R-00279`、`R-00141`、`R-00155`，记录 profile、迁移/重连和能力缺口证据。 |
| 新增关系 | 0 | Requirement 没有可写关系端点；八个依赖锚点改为写入新卡正文或审计评论（发送调度→`R-00172`/`R-00279`；Chunk 三态→`R-00151`/`R-00155`；耐久 profile→`R-00141`/`R-00228`；RTT/2→`R-00055`/`R-00281`）。 |

因此提案涉及 **27 个唯一 Requirement 对象**（22 个更新对象 + 4 个新建对象 + 1 个仅评论对象 `R-00155`；7 个评论目标中其余 6 个已包含在更新对象内），共 **33 个明确写入动作**（22 更新 + 4 新建 + 7 评论）；关闭/否决为 0。八个依赖锚点是正文/评论内容，不是额外的关系 API 写入。`MS-00001` 只是上下文，不在写入对象内。该账本不包含附件、状态流转、Room/里程碑改动或 Baseline 变更。

## 11. 本轮实际动作与证据边界

本轮 Workflow 写入为 **0**：没有 `POST`/`PATCH`、没有状态 transition、没有评论、附件、关系或新 Requirement。只读取了已保存的 JSON 快照、架构源文档和各仓 Git 锚点，并修改了本地审计附录；没有修改 `packages/`，没有 push 架构仓。

在得到 profile 和精确账本授权前，任何实现仓都不应把本附录的候选卡、百分比或建议当作已落单、已批准或已完成。

## 12. 下一步确认

先确认 `MS-00001` 采用 **bootstrap profile** 还是 **DS V1 profile**。若确认 bootstrap，再确认上面的精确对象清单和数量；若选择 DS V1，我会先按 Rust 关键路径重算账本，不沿用这组数字。

按当前推荐账本，授权问题应明确写成：

> **是否将 `lumiogamesengine` 中以下对象写入：更新 22、关闭/否决 0、新建 4、评论 7（共 27 个唯一 Requirement 对象）；关系写入 0（八个依赖锚点仅写入正文/评论）？**

在收到明确的 profile、项目和数量确认前，继续保持只读。
