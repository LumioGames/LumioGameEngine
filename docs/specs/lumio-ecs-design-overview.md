...
> 怎么吵出来的、每一板的理由、和外部调研的对账，全在[审计过账附录](2026-08-29-ecs-architecture-audit.md)与[接缝裁决流水](2026-09-01-seam-closure-decisions.md)里，本文不背这些包袱——本文只回答五个问题：**这是什么、长什么样、每块干什么、按什么顺序做、什么不许做。**
...
> 交付按 **Living Architecture**：接口真值是 `engine/abi` + `engine/wire` 契约，开发态不跑 Baseline / Fixture 门 / 七仓镜像。ADR 编号落笔时现查最高号（编号无机器占号，会被并发抢）。
