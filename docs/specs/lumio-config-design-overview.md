...
> 2026-08-30 定稿，Owner 与引擎架构总监双方认账。怎么吵出来的、每一板的理由、和旧规矩（ADR）的对账，全在[裁决流水](2026-08-30-config-table-architecture-decisions.md)里，本文不背这些包袱——本文只回答五个问题：**这是什么、长什么样、每块干什么、按什么顺序做、什么不许做。**
...
- **干什么**：本地 HTML，打开就能看能改，体验对标飞书文档。读文本表格渲染成表；还能把一个技能的主表行+子表行拼成一张「技能卡」整体看。
- **能干什么**：保存的时候不是覆盖文件，而是「跟打开时那版比，你改了哪几格」生成补丁，走 M2——所以你开着编辑器的半小时里 AI 合入的十个补丁不会被你抹掉。
- **不干什么**：它不是真身；第一期可以只做「只读查看」。
- **做完的标准**：编辑期间 AI 并行改表不丢失；保存出来的就是合法补丁。
