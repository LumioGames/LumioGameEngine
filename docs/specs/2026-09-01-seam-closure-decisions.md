...
> 实时落盘，防对话中断。产出的设计概要见 `lumio-ecs-design-overview.md` / `lumio-ds-design-overview.md` / `lumio-gas-design-overview.md` / `lumio-save-design-overview.md`。
> 本轮起因：四份「定稿 + 流水」双文件的正文掺满「怎么吵出来的」，且已被 2026-09-01 的 RM-00011 裁决部分改写而未更新。目标形态照 `lumio-config-design-overview.md`。
