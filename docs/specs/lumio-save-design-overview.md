...
> 怎么吵出来的、每一板的理由、和外部调研的对账，全在[裁决流水](2026-08-30-save-load-architecture-decisions.md)与[接缝裁决流水](2026-09-01-seam-closure-decisions.md)里，本文不背这些包袱——本文只回答五个问题：**这是什么、长什么样、每块干什么、按什么顺序做、什么不许做。**
> 配套：[ECS 设计概要](lumio-ecs-design-overview.md)（字段上的存档标记是唯一取舍依据）、[DS 设计概要](lumio-ds-design-overview.md)、[GAS 设计概要](lumio-gas-design-overview.md)、[配表设计概要](lumio-config-design-overview.md)。
...
- **能干什么**：① 提交点之后追加流水记录，**哈希链**串起来。② **实体和体素同一条链**——流水层天然就是一刀，不需要额外的协调机制。③ **变更集取样蹭复制系统已有的脏账**，不额外扫世界（零新增遍历）。④ **攒批刷盘**：攒多久刷一次 = 耐久档位这个旋钮。⑤ 磁盘回执回来后**精确清脏**（只清回执覆盖到的修订），此后才允许驱逐。
