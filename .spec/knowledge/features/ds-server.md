...
> 配套：[ECS 设计概要](ecs.md)（视野表、变更集、实体绑定的真值方）、[存档设计概要](save-load.md)、[GAS 设计概要](gas.md)、[Tick 设计概要](tick.md)（配额在哪一相读、打包在哪一相）。每连接元数据口径、阶梯第 4 级按字节、一进程一房间由 [ADR-063](../../decisions/ADR-063-architecture-review-owner-rulings-identity-persist-prediction.md) 裁决。
...
2. **diff 一次、分发多次**。变更集全服每帧只算一份；每个连接只存书签（收到哪儿了）+ 有界的可见性 / 进度记录，**绝不每连接存一份世界副本或组件值副本**。50 个人在线就是「算一次，发 50 份」，不是「算 50 次」。
3. **任何队列截断不丢弃**。带宽不够就少发，剩下的退回待发集、下一帧优先级升高；关键类别有硬饥饿上限（配额允许下最多欠 N 帧必发，装不下就进阶梯）。**没有任何队列可以偷偷扔东西。**
...
- **体素**：体素同步是独立自治的子系统，DS 图上只画引用箭头。三条接缝共享：① 同一条连接、带宽统筹（体素大件有配额，不挤死实体实时更新）；② 同一个提交点；③ 服务器侧同一个整帧作废单元（防「钻石进包了方块还在」的鬼状态）；客户端预测世界不含体素（[`gas.md`](gas.md) M7）。
...
flowchart TB
  subgraph BR["浏览器 / Bot 客户端"]
    ...
    BA["副本应用 + 预测<br/>确认世界 + 预测世界重建（体素不进预测世界）"]
  ...
  subgraph EDGE["连接与字节层"]
    ...
    T9["M9 进程、房间与维护更新<br/>一进程一版本 · 一进程一房间<br/>无进程级全局状态"]
  end
  subgraph SEM["语义与真值层"]
...
- **不干什么**：**绝不每连接存世界副本或组件值副本**（每连接只有书签 + 有界元数据）；**复制字段的值不得依赖观察者**——因人而异的信息（你是谁、你管哪个角色）在准入时告知一次、存连接侧，「只给某些人看」用可见性声明控制发不发，绝不同一字段两副面孔；不展开容器差量编码和可见性枚举（那是 ECS 的地盘）。
- **做完的标准**：50 个连接在线时，变更集计算次数为每帧 1 次（用计数器断言，不靠推理）；每连接不存任何组件值副本，常驻内存 = 书签 + 有界的可见性 / 发送进度 / 确认元数据（随该连接可见实体数线性增长，与世界总实体数无关）；把某个连接的书签人为退回到超窗位置，它收到的是 Full Resync 而不是一串补发的 Delta。
...
- **能干什么**：
  ...
  ① **优先级 = f(重要度类别, 等待时长)**：类别由内容层声明；等的越久权重越高。**关键类别有硬饥饿上限**（在本帧配额允许的前提下最多欠 N 帧必发；配额连关键类别都装不下 = 链路已不可承载，进第 3 / 4 级阶梯，不作无条件时限承诺）。
  ...
  ⑤ **慢客户端四级阶梯**，前三级的切换信号统一用**未确认 Delta 的 revision 落后深度**（不用 RTT——第三级本来就是这个量，前两级复用它就不必造第二套仪表，且双端都算得出）；第 4 级硬上限按**待发字节**（内存与 DoS 保护本来就是字节的事）：
...
- **能干什么**：① **一进程一版本**：多个进程池分别服务不同版本。② **V1 一进程一房间**：一个进程一个 World Manager 一个世界（[`ecs.md`](ecs.md) M1a、ADR-058 #11），多房间 = 多进程 + 匹配 / 路由；「Room 多槽」（进程内多个隔离房间世界、每槽独立故障域）只是回图触发项，不是 V1 能力，底层 World API 也不因此设计成进程单例。③ **无进程级全局状态**——这条今天就写死，是将来多槽的前提。④ **滚动更新**：新池接新客，旧池 Draining → Empty → Retired。⑤ 大世界形态下的「清空」= 优雅维护窗口：倒计时广播 → 快照落盘 → 维护踢出 → 新进程装快照接管 → 客户端重连强制更新（精确版本匹配，无兼容窗口）。
- **不干什么**：不做进程内多版本共存；不做无缝不停服迁移（触发条件：产品明确要求不停服）；不做跨服务器的角色转移。
- **做完的标准**：用静态检查证明零进程级可变全局状态；（多槽启用时）杀掉一个房间槽，同进程其他槽继续正常服务；滚动更新窗口期新旧进程并存，客户端按版本正确配对，零错配连接。
...
| 每连接存世界副本 / 组件值副本 | **红线** | 永不——每连接只有书签 + 有界元数据 |
...
| 书签（游标） | 每条连接收到哪儿了；服务器只存这个和一点可见性 / 进度记录，不存他那份世界 |
