...
### 判据与它的反例测试必须同一提交内诞生;「由 X 保证」的声称必须同提交 grep 自验
- 日期：2026-08-29
- 现象：一天内四例「声称与验证脱节」,其中两例是**同一提交内违反自己刚写的规范**:①CoreEngine ADR 0007 明写「只用 toml crate 解析、否决自研子集解析器」,同提交的 `toolchain.rs` 却是手写扫描;②CoreEngine ADR 0009 第 1 节写「被背书者与背书者必须不同源」,同提交却让 descriptor 的 `validatorRan`/`entrySymbol` 从被校验对象自己取回去参与重建;③R-00017 在三处源码注释里写「由仓级 `cargo tree` 断言覆盖」,而那条断言**从未创建**——`just check` 五个子项里没有依赖图断言,后续任何人给运行时闭包加一条 `test-support` 依赖都会全绿放行;④GameRuntime 修完行尾一层后,**在同一份 ADR 同一页**写下「判据成为 commit 的纯函数」,而全局 `~/.gitattributes` 仍能压过它。四例都不是粗心——**写的时候都真诚相信**。
- 根因：声称与验证之间没有机械检查。人读自己刚写的话时,读到的是意图而不是事实;而 ADR / 注释 / 卡面里的「由 X 保证」一旦写下,后人会当既成事实消费,不会回头验证。
- 规避：①**任何写进代码注释、文档或卡面的「由 X 覆盖 / 由 X 保证」声称,必须在同一提交内 `grep` 验证 X 真实存在**,并把 grep 输出留进证据评论;②**新增一条判据(ADR 条款、不变量、「由 X 保证」)时,同提交内必须有一条按该判据构造的失败用例**——判据与它的反例测试同时诞生,失同步在时间上不再可能;③注意 grep 自验只能证明「X 存在」,**证明不了「X 覆盖的范围 == 声称的范围」**(实录:「descriptor 完整性由它记的每条都对得上间接证明」——机制确实存在、grep 得到,却证明不了 descriptor 自己没被改),后半段只能靠对抗审查;④**反例的构造方式决定覆盖面**:往文件追加一个字节必然被抓,替换某字段的值则不会——同一测试意图,两种构造的盲区完全不同。
- 来源：CoreEngine R-00017 / R-00018 两轮 reviewer 退回与其 ADR 0007 / 0009 自违实录(`origin/main` 已合入);GameRuntime R-00131 的措辞收窄;LumioClient R-00017 注释与 `just check` 实测比对

### 判据与它的反例测试必须同一提交内诞生,反例的构造方式决定覆盖面
- 日期：2026-08-29
- 现象：一天两次「同一提交内违反自己刚写的规范」:①**某实现仓**的 ADR 0007 明写解析用 toml crate,同提交的实现却是手写扫描;②**某实现仓**的 ADR 0009 写「被背书者与背书者必须不同源」,同提交却让 descriptor 自报字段参与重建。(两例的仓名与卡号未随交接文档转述下来,见「来源」;本项目 ADR 编号无机器占号、跨仓存在同号,**引用这两例前须先定位所属仓**——本架构仓自己的 `ADR-007` / `ADR-009` 是另两份主题无关的 Accepted 决策,**与这两例无关**,不要读串。)两处都通过了当轮自检——因为新判据落地时**没有一条按该判据构造的失败用例**,判据从写下的那一刻起就是无人守卫的散文。同轮另有一例揭示第二面:同一测试意图,「往文件追加一个字节」的反例必然被抓,「替换某字段的值」的反例却不会,两种构造的盲区完全不同。
...
- 来源：2026-08-29 执行会话交回物的 known gaps(ADR 0007 与 ADR 0009 两例)与同日「反例构造方式决定覆盖面」的附注观察,经 TD 总调度自 `docs/plans/2026-08-29-td-handoff.md` §6「尚未升格」项升格(提交 `3631110`)。**转述缺口**:交接 §6 未记两例所属仓与卡号,升格为 `standards/` 前须补齐,否则条目不可复核。

### 「由 X 覆盖 / 由 X 保证」的声称,同提交内必须 grep 验证 X 真实存在并留证
- 日期：2026-08-29
- 现象：代码注释、文档与卡面里的「该情形由 X 覆盖 / 由 X 保证」式转嫁声称,其 X 未被机械核实——读者因为有指向而停止追问,而 X 可能并不存在、已改名或从未被引用。与「守护空转」那条不同的是:那条里守护存在但不生效,这条里守护对象本身可能是空的。
...
- 来源：2026-08-29 执行会话交回物的 known gaps,经 TD 总调度自 `docs/plans/2026-08-29-td-handoff.md` §6「尚未升格」项升格(提交 `3631110`)。**收录理由:与上条同源、合并收录**——交接 §6 只留下规则本身,未记具体实例,故本条不满足「同类问题第二次出现」的独立准入,依附上条(判据/反例)一并入池。**升格为 `standards/` 前须补齐实例的仓名与卡号**,否则按准入退出候选池。
