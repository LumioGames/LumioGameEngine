...
- 日期：2026-08-29
- 现象：一天两次「同一提交内违反自己刚写的规范」:①仓内 ADR 0007 明写解析用 toml crate,同提交的实现却是手写扫描;②仓内 ADR 0009 写「被背书者与背书者必须不同源」,同提交却让 descriptor 自报字段参与重建。(两例的仓名与卡号未随交接文档转述下来,见「来源」;本项目 ADR 编号无机器占号,跨仓存在同号,**引用这两例前须先定位所属仓**。)两处都通过了当轮自检——因为新判据落地时**没有一条按该判据构造的失败用例**,判据从写下的那一刻起就是无人守卫的散文。同轮另有一例揭示第二面:同一测试意图,「往文件追加一个字节」的反例必然被抓,「替换某字段的值」的反例却不会,两种构造的盲区完全不同。
