---
name: 2026-09-01-ecs-formal-entity-chat-decision-log
description: ECS 正式实体与聊天切片的已确认裁决台账;查某条切片决策是否已定、定成什么样时查
metadata:
  type: doc
  status: 实施中
---
...
Owner rulings from the RM-00011 room review (evidence and per-card change list: `2026-09-01-rm-00011-room-review.md`).
...
| 2026-09-01 | Room review rulings recorded: contracts-first parallel delivery, MS-00001 attachment, C# host with relay to a slice-scoped minimal Rust host, formal standalone Account Server with an ECS AccountEntity World, D-009 unblocked via ADR-049 finalization, takeover on duplicate admission, two-layer timer with real slice consumers, no R-00295 dependency, component-level persistence scope. Review report: `2026-09-01-rm-00011-room-review.md`. |
| 2026-09-01 | Owner wire landing for RM-00011 Wave 0 (public semantic): each C card is one JSON under `engine/wire/`; unified validator `eng/verify-wire.mjs`; do not extend `hello-wire-v1`; `eng/verify-hello-wire.mjs` unchanged; do not restore Schema/ID/Fixture/Baseline/seven-repo mirrors; ABI unchanged unless a card truly needs the binary boundary; downstream consumes architecture `origin/main`. Written artifact: `2026-09-01-owner-wire-landing.md`. This is the development-state equivalent of the card-literal ADR→Schema/ID→Fixture→Baseline→mirror chain, which has no remaining drop point after `59866ec`. |
| 2026-09-01 | ADR-049 Accepted as the generic gameplay command envelope (ChatInput first tenant, voxel-dig paper validation). Contract: `engine/wire/gameplay-command-envelope-v1.json` (R-00355 / C-1). |
