...
- **Published surfaces**: the profile is published in three places that must agree — `packages/canonical/canonical-digest-profile.json` (the full form, with every Golden's input and canonical bytes), `packages/rust/lumio-gen-canonical-serializer/src/lib.rs` (`CANONICAL_*`, `DIGEST_ALGORITHM` / `DIGEST_FRAMING`, `DIGEST_DOMAINS`, `CANONICAL_GOLDENS`) and `packages/csharp/Lumio.Gen.CanonicalSerializer/CanonicalProfile.cs` (`CanonicalForm`, `DigestDomains`, `CanonicalGoldens`). **The two language surfaces are asserted together**: a profile that ships in one language and not the other is not a smaller surface, it is an asymmetry the missing side cannot work around, and the consumer that hits it has no recourse inside its own repository.

## Failure semantics

A canonicalization error (non-integer number, duplicate member, unknown member) produces no bytes and therefore no digest; it is a build-time failure. A published language surface that is missing a form constant, a digest domain or a Golden — in either language — is a `validate` failure naming the constant or domain, not a diff for a reader to notice. A digest mismatch anywhere in the chain keeps the ADR-018 mapping to `ManifestDigestMismatch` / `ArtifactDigestMismatch` / `EvidenceDigestMismatch` / `SignatureInvalid` and the package never reaches the Loader.
...
**Rust surface correction.** The profile above first shipped in C# only: the generator wrote the crate's `lib.rs` before appending the form constants, the digest-domain table and the Goldens, so the Rust artifact published the snapshot-checksum constants and nothing else, and a Rust consumer could not read the digest domains from the crate it depends on. The generator now writes that file once, after the whole surface is built, and the semantic rule above holds the two languages together so the asymmetry cannot return silently. Consequences, all additive: `outputHash` moves for `canonical-serializer-rust`, and because the fix and its gate both live in the locked compiler, `compilerHash` moves for all twelve artifacts. **Consumers pinning an `outputHash` or a `compilerHash` re-pin — the one migration action this correction requires.** Nothing was removed or renamed: consumers reading only the previously published `SNAPSHOT_*` constants or the C# `CanonicalProfile` surface are unaffected, and no schema, ID, fixture or published Golden digest changes meaning, so the `LGE-V1.4-2026-08-27` baseline id stays valid.
...
Surface symmetry is proven by a control-group probe rather than by reading the generated files: permuting a published constant (`CANONICAL_NUMBERS`) and a domain tag (`CapabilitySetV1`) in `lib.rs`, and a domain tag (`ReplicationMappingSetV1`) and a Golden digest (`integer-boundary`) in `CanonicalProfile.cs`, makes `validate` fail naming exactly those four, and restoring them makes it pass — the rule bites in both directions, not only the one that was broken. Before the correction the same rule reported 25 Rust failures against 0 C# ones, which is the asymmetry stated in the shape the gate reads. `cargo check --manifest-path packages/rust/Cargo.toml` is clean on the enlarged crate.
