...
The published `digestAlgorithm` therefore carries **two** members, not one. `framing` keeps ADR-041's spelling (`PrefixFreeOverEncodedBytes`) so the shared construction is legible, and `digestInput` (`EncodedBytesOnly`) says the operative thing as data. This is not redundancy. A clean-room implementation built only from an earlier draft of this profile read `PrefixFree…` as an instruction to *prepend* something, reproduced **6/6 Golden byte strings and 0/6 digests**, and would have raised nothing had the Goldens not been there to check against. That is the D-8 failure mode — a rule that lives in a name or in prose while the consumer executes data — and the fix is the same one D-8 used: publish the discriminating fact as a field.
...
Rejections cover: integer range overflow; a negative value against an unsigned width; a fractional number; **an integral number spelled as a float** (`1.0`); a boolean against an integer; a string against an integer; **malformed hex in a byte string** (odd length, and upper case); an unknown layout kind; a missing struct field; an unknown struct field.

The last three of those exist because the clean-room check found them missing. `1.5` is refused under both a spelling-based and a value-based reading of `JsonIntegerNumbers` and therefore discriminates nothing; `1.0` is the case that separates them. Malformed hex had no published error name at all, so a clean-room implementation invented one — two conforming encoders would have disagreed silently. `valueEncoding.integerPrecision` (`ExactArbitraryPrecision`) was added for the same reason: a double-backed JSON reader — any stock JavaScript `JSON.parse`, the most likely second implementation of a profile whose vectors are spelled in JSON — rounds the `u64` Golden's `18446744073709551615` to 2⁶⁴ and then rejects a *valid* vector.

In each vector, `case` is a human-readable label and `error` is the contract. A consumer must key its conformance on `error`.
...
- `binaryForm`, `digestAlgorithm` and `valueEncoding` equal the §1–§2 freeze;
...
- **Clean-room reproduction** (the D-8 acceptance bar): an implementation built from `packages/binary/lumio-bin-profile.json` alone, with no access to this ADR, the generator or the fixtures, reproduces every Golden byte-for-byte and every digest, and refuses every rejection vector for the declared reason. The first such run reproduced 6/6 bytes, 6/6 digests and 7/7 rejections, and its findings on what remained guessable are what produced `digestInput`, `integerPrecision` and the four added rejection vectors in §3.
