...
2. **WorldEntity。** 由 World Manager 在建世界时创建，普通 CS 实体（有 NetEntityId，字段按声明同步）；存档 / Dump / Tick 配置是它组件上的字段；客户端 World 按 EntityType 取单例。世界存在 = WorldEntity 存在。
...
4. **一套源码编两份程序集，标注与宏都要。** 字段归属用 `[ServerOnly]` / `[ClientOnly]`（未标 = 两端各有本地值），生成器据此产两端各自的注册表 / 同步表，客户端程序集不含 ServerOnly 字段；逻辑块与敏感信息用 `#if LUMIO_SERVER` / `#if LUMIO_CLIENT` 物理剔除，防逆向 / 剥离。`Sync` 字段与 `[ServerOnly]` / `[ClientOnly]` 同现 = 生成期拒。`[Persist]` 哪端编译哪端存。
5. **文件布局。** 一个组件类型、按端拆 partial 文件，按组件聚合一个文件夹：`Components/<名>/X.cs`（共享：Sync 字段 + 共享逻辑）/ `X.Server.cs` / `X.Client.cs` / `X.g.cs`（生成物，入库不手改）；`EntityTypes/` 一份声明。一棵源码树、两个 csproj：`*.Server.csproj` 排除 `**/*.Client.cs` 定义 `LUMIO_SERVER`，Client 反之；生成器各跑一遍。lint：`[ServerOnly]` 只许在 `*.Server.cs`、`[ClientOnly]` 只许在 `*.Client.cs`；每文件首行注释列兄弟文件。
6. **上行参照 Unity Netcode for GameObjects。** 同步字段 = NetworkVariable 语义：`Sync<T>(scope, authority)`，默认 `Authority.Server`；`Authority.Owner` 显式 opt-in，仅绑定到该实体的连接可写自己实体的该字段，改了 `.Value` 自动上行、不写消息代码。上行字段变更与 `[ServerRpc]` 方法调用都是 ADR-049 InputCommand 信封的种类，进服务器同一条有序输入流，ApplyInputs 相按发送者 NetEntityId 排序后应用；组件可选 `OnClientWrite` 校验钩子，被拒即权威纠正推回。服务器→客户端动作 = `[ClientRpc]`。无通用 SetField RPC。存档命令 = 对 WorldEntity 的 ServerRpc。
7. **脏记账 = `Sync<T>` 包装类型（struct）。** 写 `.Value`、读隐式转换；容器 `SyncList<T>` / `SyncDict<K,V>`。标注集修订为：`[EcsComponent]`、`[Persist]`、`[ServerOnly]`、`[ClientOnly]`；不再有 `[Replicate]` / `[Visibility]`（类型即声明）。生成器只产表与内部模板类，不在用户类型上生成成员。下行写入走 Sync 内部接口，不记脏不回声。
8. **读。** 玩法代码只有类型化读：`Get<T>()` 读自己、`Get<T>(id)` 读别人、`world.Each<T>()` 系统遍历——一种写法，不用知道对方实体类型。C-2 AttributeId 查询面保留但退成生成的薄适配层（字符串名 → 同一世界同一 Sync 字段），供宿主探针 / 验收 / 工具，无自有存储；四种结局由世界状态派生。
...
12. **生成桥。** 一个生成命令产三件：组件注册表 + 实体模板类（`.g.cs`）、同步表、C-2 契约声明表（json）；扩现有 `gen-declarations`，做成 MSBuild 目标每次 build 自动跑（秒级增量）；生成物入库、测试断言零 diff；非法组合生成时拒；世界只收生成注册表，手写注册与反射破门删除。EntityType 声明式：`[EntityType(Mode.CS)] [Has(typeof(X))]… [Child("Weapon", typeof(WeaponEntity))] public static class PlayerEntity {}`。**开发期热重载（仅开发构建）**：`dotnet watch`；改方法体 = .NET Hot Reload 原地生效；改字段 / `[Persist]` / Scope / EntityType 组件集 = 生成器重跑 → 世界热重载 = 快照 → 换程序集 → `CreateFromSnapshot`，新字段取默认值，进程不重启、连接不断；改 wire 契约 = 重新握手。不用 Roslyn 源生成器。
...
14. **绑定 = 实体字段 + 派生索引，无独立绑定表。** `IdentityComponent`：`[Persist] AccountId`（Server.cs）、`Sync<EntityKind> Kind`（Room）、`Connected / ConnectionGeneration / DisconnectedAtTick`（Server.cs，不存档，重启即离线）。Manager 维护可从世界重建的 accountId → NetEntityId 索引；宿主只持连接 → NetEntityId 会话表；顶号 = 查实体 Connected；过期 = DisconnectedAtTick + 内核定时；C-2 五元组由实体字段 + 宿主会话表拼出。
...
17. **客户端 World。** Client 直接引用 Runtime ECS（`Lumio.GameRuntime.Ecs` + 复制客户端模块 + 玩法 `*.Client` 程序集，全部 netstandard2.1）；删 `ReplicaWorld` 属性袋与手写 `AttributeDeclarationTable`；客户端 World 由同一 World Manager 类以 Client 模式建（收世界变化消息进客户端提交相、不发号、不存世界档、字段上行只按 `Authority.Owner`）；Bot.Host 用同一客户端 World。
18. **事件。** 字段 = 最后状态（可存可查可同步）；事件 = 一次性通知 = `[ClientRpc]`（不存不查不回放）。事件在提交相发出、与字段变化同一 Tick 包下发，投影后服务器即丢（每 Tick outbox）；可靠有序由每连接有界传输队列 + 游标（宿主）保证；重连全量快照不回放；客户端聊天窗口 = 客户端 World 的 ClientOnly 状态；Runtime 内 `_displayed` 与 `_eventsByRoomTick` 删除。

样板示例（用户名的声明 → 建世界 → 创建 → 写 → 同步 → 读 → 存档全链路）落 `knowledge/features/ecs.md` §4.5，作为以后所有 ECS 代码与讨论的标准样例。
...
- `[ServerOnly]` 出现在非 `*.Server.cs`、`[ClientOnly]` 出现在非 `*.Client.cs`——lint 失败。
- `Sync` 字段与 `[ServerOnly]` / `[ClientOnly]` 同现、EntityType 缺必需组件 / 依赖成环、阶梯外类型——生成命令失败，不留到运行时。
...
- `knowledge/features/ecs.md` M1 / M2 / M3 / M4 / M8 / M9 按本 ADR 改写，新增 M1a「World Manager 与 WorldEntity」与 §4.5 样板示例；「ReplicaWorld」全部改「客户端 World」；八回调改九回调（加 PostAttribute）。
...
2. **生成三件**：一句命令产注册表 / 同步表 / 声明表，入库零 diff；手写注册与反射破门不存在；`[ServerOnly]` 放错文件 lint 失败。
...
6. **客户端同套 ECS**：Client csproj 引用 Runtime ECS，属性袋与手写声明表不存在；创建记录按 EntityType 建实体，探针确认 Awake → PostAttribute → Start。
