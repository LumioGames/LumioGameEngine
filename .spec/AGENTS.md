...
- **并行边界与合入:** 任务文件集**互不重叠**才可并行(最小化冲突),重叠必串行;拆解产物按 wave 分批扇出,批间串行。并行 worker 各在独立 git worktree 实现(Claude Code 用 Agent 工具的 worktree 隔离),reviewer 审 worktree 相对基线的完整 diff,通过后主 loop 合入主工作区,未过审不合入,冲突退回实现方。**reviewer 同样须在独立 worktree 里跑验证**,主 loop 派审后停止在同一 worktree 跑构建——同 worktree 并发构建会互锁(MSBuild 节点复用 + `obj/` 争用,.NET 侧已实测),两侧都会假失败。多宿主并存时共享任务真值是 `.spec/tasks/`,宿主内置任务工具只作个人草稿。
