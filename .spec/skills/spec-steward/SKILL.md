...
2. 判断文档归属：
   ...
   - **决策**（功能内与框架级都算：为什么这样调度 / 选型 / 划边界）→ `decisions/` ADR（**全仓唯一落点**，见 `decisions/README.md`）；本架构仓 `Draft` 可随验证修订，`Accepted` 只能新增 ADR 取代。
...
| 决策（功能内 / 框架级，唯一落点） | `decisions/`（ADR；沿用 `ADR-NNN-<slug>.md`） | 无（见其 README 模板） |
