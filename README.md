...
Per ADR-048 those artifacts carry **bodies, not just catalogs**. `ContractTypes` publishes generated C# and Rust types for the eight closed contracts (`ConfigTable`, `ProcessorDescriptor`, `TxnJournalRecord`, `CommandLogRecord`, `WalRecordEnvelope`, `EntityIdentity`, `ReplicationEnvelope`, `SessionRevisionVector`) with **fields in schema declaration order** — `allOf` members first, then own properties — and each type republishes that order so `validate` can reject a type whose order has drifted from its schema. Enum ordinals come from `ids/index.json` where a registry owns the names (`messageType` uses the `MessageType` numerics, not the enum's position) and from declaration order otherwise; every emitted enum states which. Objects the architecture source leaves open (a replication `body`, a WAL `inner`, a config row's `values`) are carried verbatim as `OpaqueJson` rather than given an invented shape. `ProtocolPermissionValidator` publishes the **executable** ADR-022 gate: it returns a verdict and a reason, with the rejection precedence published as data, and every `protocol-permission-gate` fixture is run through it rather than asserted about. Every generated C# project multi-targets `netstandard2.1;net8.0` so Unity and the .NET Host consume one package. `packages/abi/lumio_core.h`, `root_abi.rs` and `RootAbi.cs` publish the ADR-040 §7.1 capability keys projected from the ID Registry — `ids/index.json` stays the authority, the generator is the only emitter, and `validate` cross-checks all three forms against it.
