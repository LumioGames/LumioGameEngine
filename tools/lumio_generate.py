...
# Every generated crate carries this version, and `Cargo.lock` must repeat it
# for each workspace member -- one constant so the manifest and the lockfile
# cannot drift apart.
RUST_CRATE_VERSION = "0.0.0"

# Cargo lockfile format, not a crate version. Cargo keeps an existing lockfile
# at its recorded version and only rewrites the file when the recorded content
# is wrong, so emitting v4 is stable across newer toolchains. If a future Cargo
# ever does rewrite it, `published_cargo_lock_errors()` fails the next
# `validate` rather than letting the rewrite ride along unnoticed.
RUST_LOCK_VERSION = 4
...
def rust_cargo(name: str) -> str:
    return (
        ...
        "publish = false\n\n"
        "[dependencies]\n"
    ).format(name, RUST_CRATE_VERSION)


def workspace_lock() -> str:
    """The workspace `Cargo.lock`, byte-for-byte as Cargo would write it.

    The six generated crates declare no dependencies, so the lockfile is a pure
    function of the member names -- Cargo sorts `[[package]]` blocks by name.
    Emitting it here rather than leaving it to a `cargo` run is what keeps
    `generate --out packages` from deleting a tracked file: `command_generate`
    rmtree's the output directory, so anything the generator does not emit is
    staged as a deletion by whoever regenerates without running Cargo first.
    """
    blocks = "\n".join(
        "[[package]]\nname = \"{}\"\nversion = \"{}\"\n".format(name, RUST_CRATE_VERSION)
        for name in sorted(RUST_CRATES.values())
    )
    return (
        "# This file is automatically @generated by Cargo.\n"
        "# It is not intended for manual editing.\n"
        "version = {}\n\n".format(RUST_LOCK_VERSION)
    ) + blocks
...
def emit_workspace(rust_root: Path) -> None:
    ...
    # Both workspace files sit above the crate directories, so neither is read
    # by `dir_output_hash` and emitting the lockfile moves no `outputHash`.
    write_text(rust_root / "Cargo.lock", workspace_lock())
