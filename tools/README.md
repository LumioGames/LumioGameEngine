...
The `canonical` command emits Decimal-valued GAS evaluation members as JSON
number tokens without a binary-float conversion, so canonicalized evaluation
records retain their exact decimal values on round-trip.
...
For GAS A1, the semantic gate checks the frozen lifecycle event table, Admission/Commit order and outcomes, single charging, exact-lexeme Decimal34 arithmetic and adjusted-exponent bounds, deterministic evaluation/override selection, and same-Tick Effect event order. The `canonical` command parses decimal numbers exactly regardless of input path and rejects duplicate members and non-finite constants. GAS invalid fixture metadata is matched by registered rule key, never by a substring. The schemas remain the public data contract; this tool is the single architecture validator for cross-field invariants and is not a Runtime implementation.
